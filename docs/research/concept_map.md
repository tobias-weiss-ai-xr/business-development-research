# Concept Map — Knowledge Graph of the Corpus

**Generated:** 38 concepts, 188 relationships from 6444 papers. Edges = co-occurrence (shared papers).

## Top concepts

| Concept | Kind | Papers |
|----------|------|--------|
| Ai Adoption | taxonomy | 773 |
| framework | curated | 725 |
| Digital Marketing | taxonomy | 469 |
| Pricing Strategy | taxonomy | 402 |
| Market Analysis | taxonomy | 384 |
| Content Marketing | taxonomy | 380 |
| Go To Market | taxonomy | 359 |
| Entrepreneurship | taxonomy | 313 |
| Competitive Intelligence | taxonomy | 287 |
| Customer Retention | taxonomy | 277 |
| Customer Acquisition | taxonomy | 273 |
| Sales Strategy | taxonomy | 265 |
| Business Models | taxonomy | 253 |
| B2b Marketing | taxonomy | 244 |
| survey | curated | 234 |
| Customer Success | taxonomy | 208 |
| Growth Metrics | taxonomy | 207 |
| Partnerships | taxonomy | 205 |
| dataset | curated | 188 |
| case study | curated | 131 |
| quantitative | curated | 125 |
| qualitative | curated | 123 |
| benchmark | curated | 112 |
| literature review | curated | 104 |
| experiment | curated | 100 |
| systematic review | curated | 69 |
| taxonomy | curated | 38 |
| interview | curated | 14 |
| reproducibility | curated | 13 |
| peer review | curated | 8 |
| research gap | curated | 7 |
| triangulation | curated | 3 |
| mixed methods | curated | 2 |
| grounded theory | curated | 2 |
| meta analysis | curated | 1 |
| open access | curated | 1 |
| Survey | taxonomy | 0 |
| Method | taxonomy | 0 |

## Relationships (top concepts → related concepts)

### Ai Adoption

- framework — 195 shared paper(s)
- survey — 65 shared paper(s)
- qualitative — 53 shared paper(s)
- dataset — 45 shared paper(s)
- quantitative — 36 shared paper(s)
- experiment — 25 shared paper(s)
- literature review — 24 shared paper(s)
- benchmark — 23 shared paper(s)

### framework

- Ai Adoption — 195 shared paper(s)
- benchmark — 49 shared paper(s)
- Digital Marketing — 49 shared paper(s)
- Content Marketing — 45 shared paper(s)
- dataset — 41 shared paper(s)
- survey — 32 shared paper(s)
- Go To Market — 32 shared paper(s)
- case study — 31 shared paper(s)

### Digital Marketing

- framework — 49 shared paper(s)
- dataset — 22 shared paper(s)
- benchmark — 11 shared paper(s)
- literature review — 10 shared paper(s)
- systematic review — 9 shared paper(s)
- survey — 8 shared paper(s)
- case study — 7 shared paper(s)
- experiment — 5 shared paper(s)

### Pricing Strategy

- framework — 30 shared paper(s)
- survey — 26 shared paper(s)
- benchmark — 12 shared paper(s)
- experiment — 11 shared paper(s)
- dataset — 7 shared paper(s)
- case study — 4 shared paper(s)
- quantitative — 3 shared paper(s)
- qualitative — 3 shared paper(s)

### Market Analysis

- survey — 36 shared paper(s)
- framework — 31 shared paper(s)
- systematic review — 16 shared paper(s)
- case study — 15 shared paper(s)
- dataset — 10 shared paper(s)
- literature review — 9 shared paper(s)
- quantitative — 8 shared paper(s)
- qualitative — 6 shared paper(s)

### Content Marketing

- framework — 45 shared paper(s)
- dataset — 21 shared paper(s)
- benchmark — 13 shared paper(s)
- quantitative — 6 shared paper(s)
- taxonomy — 6 shared paper(s)
- case study — 6 shared paper(s)
- survey — 5 shared paper(s)
- experiment — 5 shared paper(s)

### Go To Market

- framework — 32 shared paper(s)
- survey — 21 shared paper(s)
- qualitative — 9 shared paper(s)
- dataset — 7 shared paper(s)
- quantitative — 7 shared paper(s)
- experiment — 5 shared paper(s)
- case study — 4 shared paper(s)
- literature review — 3 shared paper(s)

### Entrepreneurship

- framework — 14 shared paper(s)
- survey — 11 shared paper(s)
- case study — 5 shared paper(s)
- qualitative — 5 shared paper(s)
- taxonomy — 4 shared paper(s)
- dataset — 4 shared paper(s)
- systematic review — 4 shared paper(s)
- quantitative — 3 shared paper(s)

### Competitive Intelligence

- framework — 17 shared paper(s)
- benchmark — 8 shared paper(s)
- literature review — 8 shared paper(s)
- dataset — 7 shared paper(s)
- quantitative — 5 shared paper(s)
- qualitative — 4 shared paper(s)
- survey — 4 shared paper(s)
- experiment — 2 shared paper(s)

### Customer Retention

- framework — 31 shared paper(s)
- dataset — 20 shared paper(s)
- benchmark — 4 shared paper(s)
- literature review — 3 shared paper(s)
- experiment — 2 shared paper(s)
- case study — 2 shared paper(s)

### Customer Acquisition

- framework — 21 shared paper(s)
- case study — 10 shared paper(s)
- dataset — 7 shared paper(s)
- survey — 4 shared paper(s)
- systematic review — 2 shared paper(s)
- experiment — 2 shared paper(s)
- literature review — 2 shared paper(s)

### Sales Strategy

- framework — 9 shared paper(s)
- case study — 6 shared paper(s)
- dataset — 3 shared paper(s)
- literature review — 3 shared paper(s)
- quantitative — 2 shared paper(s)
- qualitative — 2 shared paper(s)
- survey — 2 shared paper(s)

### Business Models

- framework — 23 shared paper(s)
- case study — 7 shared paper(s)
- quantitative — 7 shared paper(s)
- experiment — 6 shared paper(s)
- systematic review — 5 shared paper(s)
- literature review — 4 shared paper(s)
- qualitative — 3 shared paper(s)
- survey — 2 shared paper(s)

### B2b Marketing

- survey — 22 shared paper(s)
- framework — 16 shared paper(s)
- quantitative — 9 shared paper(s)
- case study — 8 shared paper(s)
- benchmark — 2 shared paper(s)
- literature review — 2 shared paper(s)
- dataset — 2 shared paper(s)
- experiment — 2 shared paper(s)

### survey

- Ai Adoption — 65 shared paper(s)
- Market Analysis — 36 shared paper(s)
- framework — 32 shared paper(s)
- Pricing Strategy — 26 shared paper(s)
- experiment — 25 shared paper(s)
- B2b Marketing — 22 shared paper(s)
- Go To Market — 21 shared paper(s)
- quantitative — 19 shared paper(s)

### Customer Success

- framework — 10 shared paper(s)
- case study — 7 shared paper(s)
- quantitative — 7 shared paper(s)
- survey — 3 shared paper(s)
- literature review — 3 shared paper(s)
- qualitative — 2 shared paper(s)

### Growth Metrics

- framework — 28 shared paper(s)
- dataset — 7 shared paper(s)
- case study — 6 shared paper(s)
- literature review — 5 shared paper(s)
- benchmark — 3 shared paper(s)
- quantitative — 3 shared paper(s)
- survey — 3 shared paper(s)
- taxonomy — 2 shared paper(s)

### Partnerships

- framework — 8 shared paper(s)
- literature review — 8 shared paper(s)
- case study — 4 shared paper(s)
- qualitative — 4 shared paper(s)
- dataset — 2 shared paper(s)

### dataset

- Ai Adoption — 45 shared paper(s)
- framework — 41 shared paper(s)
- Digital Marketing — 22 shared paper(s)
- benchmark — 21 shared paper(s)
- Content Marketing — 21 shared paper(s)
- Customer Retention — 20 shared paper(s)
- Market Analysis — 10 shared paper(s)
- survey — 8 shared paper(s)

### case study

- framework — 31 shared paper(s)
- Ai Adoption — 16 shared paper(s)
- Market Analysis — 15 shared paper(s)
- Customer Acquisition — 10 shared paper(s)
- B2b Marketing — 8 shared paper(s)
- Digital Marketing — 7 shared paper(s)
- Customer Success — 7 shared paper(s)
- Business Models — 7 shared paper(s)

### quantitative

- Ai Adoption — 36 shared paper(s)
- framework — 30 shared paper(s)
- qualitative — 25 shared paper(s)
- survey — 19 shared paper(s)
- B2b Marketing — 9 shared paper(s)
- Market Analysis — 8 shared paper(s)
- Go To Market — 7 shared paper(s)
- Customer Success — 7 shared paper(s)

### qualitative

- Ai Adoption — 53 shared paper(s)
- framework — 25 shared paper(s)
- quantitative — 25 shared paper(s)
- Go To Market — 9 shared paper(s)
- survey — 8 shared paper(s)
- experiment — 7 shared paper(s)
- Market Analysis — 6 shared paper(s)
- dataset — 5 shared paper(s)

### benchmark

- framework — 49 shared paper(s)
- Ai Adoption — 23 shared paper(s)
- dataset — 21 shared paper(s)
- Content Marketing — 13 shared paper(s)
- Pricing Strategy — 12 shared paper(s)
- Digital Marketing — 11 shared paper(s)
- Competitive Intelligence — 8 shared paper(s)
- taxonomy — 4 shared paper(s)

### literature review

- Ai Adoption — 24 shared paper(s)
- framework — 10 shared paper(s)
- Digital Marketing — 10 shared paper(s)
- Market Analysis — 9 shared paper(s)
- Partnerships — 8 shared paper(s)
- Competitive Intelligence — 8 shared paper(s)
- systematic review — 5 shared paper(s)
- Growth Metrics — 5 shared paper(s)

### experiment

- Ai Adoption — 25 shared paper(s)
- survey — 25 shared paper(s)
- framework — 13 shared paper(s)
- Pricing Strategy — 11 shared paper(s)
- qualitative — 7 shared paper(s)
- Business Models — 6 shared paper(s)
- Go To Market — 5 shared paper(s)
- Digital Marketing — 5 shared paper(s)


Generated by `tools/relate_concepts.py` — do not edit by hand.